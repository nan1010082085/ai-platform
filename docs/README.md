...
- Agent Workflow REST API + Webhook + **Open API（API Key）**
- `@schema-platform/workflow-client` SDK
- SDK 独立使用（`@ai-sdk`）
- 事件协议（30+ 事件类型）
...
- [插件中心](./plugin-registry.md) — Expert / Skill / Tool / MCP 分目录配置
- [插件路线图](./plugin-roadmap.md) — 未完成项 + Workflow 对外开放规划
...
- [Workflow 开放 API](./design/workflow-open-api.md) — 外部系统调用、API Key、SSE、SDK
